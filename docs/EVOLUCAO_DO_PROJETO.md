...
1. **O stack adotado existia só no `.env.example`; os defaults do código eram a era anterior.** — ✅ *resolvido na [#81](https://github.com/danielpvp22/mente_digital/pull/81), depois deste levantamento.* Três parâmetros que formam um conjunto indivisível divergiram do que o projeto de fato usa. Em `config.py`: `embedding_model` = MiniLM, prefixos vazios, `rag_score_confident` = `0.8` — todos inalterados desde o commit inicial (`5919df3`, 14/jul). No `.env.example`, desde 25/jul (`1f7cfed`): e5-base, prefixos `query:`/`passage:`, gate `0.16` (derivado por `eval/calibrar_gate.py` quando o embedding foi trocado em `0d345c4`).

   Vale a nuance: os defaults do código são **coerentes entre si** — é o stack MiniLM completo, não uma mistura quebrada. O custo de rodar sem `.env` é usar silenciosamente o embedding **antigo** (~2× pior no ranqueamento) com um gate `0.8` mais frouxo até que o `0.55` que era o valor operacional daquela era.

   O risco maior é para quem edita **um** dos três. Gate `0.16` com MiniLM **rejeita quase tudo** e manda cada pergunta para a web; embedding e5 com gate `0.8` faz o oposto. É a assinatura exata do bug L2-vs-cosseno da fundação — um limiar medido numa escala aplicado a distâncias de outra. Existe rede de segurança para o índice (`_fingerprint_ok` detecta embedding/prefixo divergentes e reconstrói do vault), mas **nenhuma para os limiares**. A correção certa é promover os quatro defaults **juntos**, não só o gate.

   **Desfecho.** Foi o que a #81 fez. E o conserto revelou um segundo achado da mesma família: **nove testes** de gate/malha/figura quebraram sem que uma linha de lógica mudasse, porque afirmavam a *escala* (`0.4` = "confiante") em vez do *comportamento*. Passaram a derivar as distâncias sintéticas do limiar vigente (`dist_confiante`/`dist_longe`), então a próxima troca de embedding não os quebra.

2. **A documentação estava defasada em relação ao código.** — ✅ *resolvido na [#79](https://github.com/danielpvp22/mente_digital/pull/79), que é a PR deste relatório.* O README (última atualização 25/jul) declarava "~12.900 linhas em 34 módulos + 885 testes" quando o real já era o dobro. É um sintoma do ritmo, não de descuido — o histórico mostra ressincronizações periódicas de contagem, só que a última ficou para trás. **Segue em aberto:** a descrição do repositório no GitHub ainda cita 824 testes, e ela é configuração do GitHub, não um arquivo versionado.
