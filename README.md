...
| **Pydantic Settings** | Configuração | **101 parâmetros** com prefixo `MENTE_`, todos com default derivado de `BASE_DIR`. Calibrar o sistema — inclusive todos os botões dos agentes — **nunca** exige editar código. |
| **HTML/CSS/JS puro** | Frontend | SPA de arquivo único (465 linhas), sem framework e sem build. A fila de áudio tem 3 linhas — porque o wire é WAV base64. Uma bolha própria com 🔔 abre para as mensagens `{tipo: proativo}`. |
| **pytest** | Testes | **472 testes que rodam sem GPU e sem rede** (de 80), com fakes de LLM/TTS/store e clock injetado. Testabilidade aqui é restrição de design, não add-on. |

---

## 🗂 Papel de cada módulo

**~8.400 linhas de Python** em 20 módulos (de ~3.300 em 12), mais os testes e ~465 de frontend. Nenhum módulo de domínio conhece o WebSocket: o pipeline recebe um callback `send(dict) -> bool` e é só isso que ele sabe do mundo exterior.

```
mente_digital/
├── main.py         # 173  wiring: lifespan monta o AppContext, sobe o scheduler, rotas + WS
├── config.py       # 427  Settings (Pydantic), 101 knobs + dicionário fonético do TTS
├── prompts.py      # 369  todos os prompts de sistema/tarefa + as tags Zettelkasten
├── state.py        # 205  AppContext (DI) + SessionMemory (histórico + estado dos agentes)
...
├── rag.py          # 1024 EmbeddingProvider + VectorStore + MalhaIndex + WebSearcher
├── agent.py        # 2104 pipeline de resposta, _fluxo_mestre, tools, ETL, síntese, conexões
├── tools.py        # 708  function calling aditivo: gate, roteador JSON, agentes de agenda/lista
├── mestre.py       # 836  PALAVRA-MESTRE: plano de comando isolado e determinístico
├── agenda.py       # 199  parser de tempo PT-BR puro (relativo/absoluto/recorrente)
├── scheduler.py    # 310  SchedulerService: alarmes, watchers, briefing, pomodoro (persistente)
...
├── ws.py           # 249  LiveSession: VAD, barge-in, conversas, PUSH proativo, fim de sessão
├── telemetry.py    # 810  logs coloridos thread-safe + Database (SQLite, todas as tabelas)
├── templates/      # index.html — a SPA inteira (465 linhas)
├── modelos/        # LLM .gguf + voz Piper + whisper/ (binários fora do git)
└── tests/          # 472 testes, sem GPU, sem rede
```
...
836 linhas quase todas puras: `separar`, `parse_rapido`, `parse_composto`/`dividir_comandos`, `comando_desfazer`/`reverter`, `tem_correcao`/`parse_correcao`/`refazer_com`, `comando_confirmar`/`_abortar`, `parse_atalho`, `parse_gatilho`, `comando_conexoes`. O instante de referência é sempre injetado.
...
### `srs.py` / `habitos.py` / `grafo.py` / `calendario.py` — os módulos-agente puros
Minúsculos e testáveis por design: `srs.py` (Leitner, intervalos de repetição espaçada), `habitos.py` (cálculo de streak), `grafo.py` (pontes por **surpresa** = 1 − Jaccard das vizinhanças, domínios disjuntos primeiro), `calendario.py` (parser mínimo de `VEVENT` do `.ics`). Nenhum toca GPU, rede ou disco — recebem dados e devolvem dados.
...
Logs coloridos thread-safe e o wrapper de SQLite — agora com **todas as tabelas dos agentes** e suas migrações idempotentes. Histórico agrupado **por conversa**, `save_latency` por resposta, e a regra do projeto codificada: **nunca `except: pass`**.
...
**A justificativa real é coabitação.** A RTX 3080 tem 10 GB para: pesos + KV-cache de `n_ctx=8192` + embeddings na GPU + eventualmente Whisper. A mesma restrição explica, em cascata: `whisper_device="cpu"`, `flash_attn=True` (invertendo o default), `kv_cache_type="f16"` com `q8_0` como escape hatch documentado.
...
pytest                    # 472 testes, sem GPU e sem rede
```
...
**101 parâmetros** vivem em [`config.py`](config.py), sobrescrevíveis por `.env` com prefixo `MENTE_`. **Calibrar o sistema nunca exige editar código.** Guia completo em `CALIBRACAO.md`. Os mais úteis:

| Variável | Default | Efeito |
|---|---|---|
| `MENTE_RAG_SCORE_CONFIDENT` | `0.8` | **O principal botão.** Distância abaixo da qual um match vale sem casar keyword. Menor = mais rígido = mais web |
...
| `MENTE_KV_CACHE_TYPE` | `f16` | `q8_0` corta ~metade da VRAM de KV. **Exige `flash_attn=True`** |
| `MENTE_WHISPER_MODEL` | `small` | `large-v3` para máxima qualidade |
...
| `/api/metrics` | GET | ETL por status, **médias de TTFT/TTFA**, e prontidão de cada serviço |
...
> **Nota que reforça a fronteira do port:** migrar de LLM local para API (ou vLLM/ExLlamaV3) toca **apenas** o `LlamaManager`. O resto só conhece `stream()` e `collect()` — e a prova está no `conftest.py`: o `FakeLlama` tem exatamente **dois métodos**. **A suíte de 472 testes é a evidência empírica de que a abstração vaza pouco.**
...
| Ponto | Situação |
|---|---|
| **Números de TTFT/TTFA publicados** | O instrumento existe (`/api/metrics`) mas o README ainda não publica médias por rota. É a lacuna mais visível num projeto cuja tese é latência percebida |
| **Escolha do modelo** | `Qwen2.5-Coder-7B` é herdado do MVP e nunca foi comparado com um base num benchmark de PT-BR. O único ponto que parece herdado em vez de decidido |
| **CI** | 472 testes que rodam sem GPU nem rede — literalmente o cenário de GitHub Actions gratuito. Um badge verde converteria "é testável" em fato verificável |
| **Calibração dos agentes na base real** | Os botões da Onda 3 (`ATERRAMENTO_IDF_MIN`, `MALHA_SIM_MIN`, os mínimos de atalho/conexão) têm defaults sensatos mas ainda faltam ajustar contra uso prolongado — ver `CALIBRACAO.md` |
